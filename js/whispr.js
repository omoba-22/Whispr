...
const SUPABASE_URL = 'https://scczitcxqazohgbipogy.supabase.co';
const SUPABASE_KEY = 'sb_publishable_lfNQ4fzNs25bKpz5hQCGBA_pP2WfhZd';
...
function getSessionId() {
  let id = localStorage.getItem('whispr_sid');
  if (!id) {
    id = Math.random().toString(36).slice(2) + Date.now().toString(36);
    localStorage.setItem('whispr_sid', id);
  }
  return id;
}
const SESSION_ID = getSessionId();

/* Normalize: trim + lowercase + no spaces */
function normalize(str) { return (str || '').trim().toLowerCase().replace(/\s+/g, ''); }

/* Simple FNV-1a hash for secret codes */
...
/* ── LIKES ── */
function isLiked(id) {
  return JSON.parse(localStorage.getItem('whispr_likes') || '[]').includes(id);
}
...
async function getRepliesUsed() {
  ...
  try {
    const res  = await fetch(`${SUPABASE_URL}/rest/v1/reply_counts?session_id=eq.${SESSION_ID}&month_key=eq.${monthKey}&select=count`, { headers: BASE_HEADERS });
    const data = await res.json();
    return data.length ? (data[0].count || 0) : 0;
  } catch(e) { return 0; }
}

/* Check if user has unlimited pass */
function hasUnlimitedPass() {
  const pass = JSON.parse(localStorage.getItem('whispr_pass') || 'null');
  if (!pass) return false;
  if (pass.type === 'unlimited') {
    // Check expiry (1 month)
    return Date.now() < pass.expires;
  }
  return false;
}

/* Get remaining paid replies */
function getPaidReplies() {
  const pack = JSON.parse(localStorage.getItem('whispr_pack') || 'null');
  if (!pack) return 0;
  return pack.remaining || 0;
}
...
function showToast(msg) {
  ...
  if (!t) {
    ...
    t.id = 'whispr-toast';
    t.style.cssText = 'position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%) translateY(200px);background:#1e1e2e;border:1px solid #1e1e2e;color:#e8e8f0;padding:0.6rem 1.1rem;border-radius:999px;font-size:0.8rem;font-weight:500;z-index:9999;transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1),opacity 0.3s;white-space:nowrap;pointer-events:none;opacity:0;font-family:DM Sans,sans-serif;';
...
function renderCard(msg, i, opts = {}) {
  ...
  const recipientLine = inInbox
    ? `<div class="msg-meta">sent to <span class="msg-meta-name">you</span></div>` : '';

  const rankHTML = showRank
    ? `<div class="trending-rank ${rankClass}">${showRank}</div>` : '';

  // Reply thread UI (shown in feed when message has a reply)
  let replyThreadHTML = '';
    ...
    replyThreadHTML = `
      <div class="reply-thread">
        ...
        <div class="reply-thread-bubble">
          <div class="reply-thread-label">replied</div>
          <div class="reply-thread-text">${escapeHTML(msg.reply_text)}</div>
        </div>
      </div>`;
  }

  // Inbox swipe hint
  const swipeHint = inInbox && !msg.reply_text
    ? `<div class="swipe-hint">swipe right to reply →</div>` : '';

  const msgEsc  = escapeHTML(msg.message).replace(/'/g,"\\'").replace(/\n/g,' ');
  const moodEsc = escapeHTML(msg.mood || '').replace(/'/g,"\\'");

  // Reply badge for inbox
  const replyBadge = inInbox && msg.reply_text
    ? `<span class="replied-badge">replied ✓</span>` : '';

  return `
    <div class="msg-card msg-card-${key}" data-mood="${escapeHTML(msg.mood)}" data-id="${msg.id}" style="animation-delay:${delay}s" ${inInbox ? `data-swipe="true" data-msg-id="${msg.id}" data-msg-text="${msgEsc}" data-msg-mood="${moodEsc}"` : ''}>
      ${rankHTML}
      <div class="msg-top">
...
async function toggleLike(btn, id, currentLikes) {
  ...
  const newVal   = !liked;
  const newCount = newVal ? currentLikes + 1 : Math.max(0, currentLikes - 1);
  btn.classList.toggle('liked', newVal);
  btn.querySelector('.like-count').textContent = newCount;
  setLiked(id, newVal);
  try {
    await dbPatch('messages', `?id=eq.${id}`, { likes: newCount });
    btn.setAttribute('onclick', `toggleLike(this,'${id}',${newCount})`);
  } catch(e) {
...
async function shareAsImage(message, mood) {
  ...
  try {
    ...
    canvas.toBlob(async (blob) => {
      ...
      if (navigator.share && navigator.canShare?.({ files: [new File([blob],'whispr.png',{type:'image/png'})] })) {
        await navigator.share({ files: [new File([blob],'whispr.png',{type:'image/png'})], title: 'Whispr' });
      } else {
...
/* ── SWIPE TO REPLY (attach to inbox cards) ── */
  ...
  container.querySelectorAll('[data-swipe="true"]').forEach(card => {
    ...
    function onMove(e) {
      ...
      if (currentX > 0) {
        ...
        card.style.opacity = `${1 - currentX / 300}`;
      }
    }
    function onEnd() {
      if (!dragging) return;
      ...
      if (currentX > 70) {
        ...
        setTimeout(() => {
          ...
          onSwipeRight({
            ...
            text: card.dataset.msgText,
            mood: card.dataset.msgMood
          });
        }, 250);
      } else {
        card.style.transform = '';
...
const SHARED_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .like-btn svg { width: 13px; height: 13px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; transition: fill 0.2s; }
  ...
  @media (max-width: 640px) {
    ...
    .page-wrap { padding: 0.9rem 0.9rem calc(var(--bottom-nav-h) + 2rem); }
  }
